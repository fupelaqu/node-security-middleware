...
/*UsernamePasswordToken.prototype.getPassword = function(){
    return new String(this.password.join(''));
};*/
 ...
 * Class SimpleAccount
 */
var SimpleAccount = function(principal, credentials, realm){
...
var Realm = function(options){
    ...
    this.realm = options.realm || 'Authorization Required';
...
Realm.prototype.authenticate = function(token){
    if(utils.isUndefined(token) 
        || utils.isUndefined(token.getUsername) 
        || utils.isUndefined(token.getPassword)){
        throw new Error('wrong token');
    }
    var username = token.getUsername();
    // Null username is invalid
    if(utils.isUndefined(username)){
        throw new Error('Null usernames are not allowed by this realm.');
    }
    // Get the user with the given username. If the user is not
    // found, then they don't have an account and we throw an
    // exception.
    var user = this.store.lookup(username);
    if(utils.isUndefined(user)){
        throw new Error('No account found for user ' + username);
    }
    // Now check the user's password against the hashed value stored
    // in the store.
    var account = new SimpleAccount(username, user.password, this.realm);
...
Realm.prototype.isPermitted = function(principal, requiredPermission) {
    ...
    var existsRequiredPermission = function(privileges){
        ...
        var found = match(privileges, search);
        if(!found){
 ...
 * Class Subject
 */
var Subject = function(realm, req, secure){
    ...
    if(utils.isUndefined(account)){
        ...
        if(utils.isDefined(account) && utils.isEqual(realm.realm, account.realm)){
            this.account = new SimpleAccount(account.principal, null, realm.realm);
            console.log(
    ...
    else if(utils.isEqual(realm.realm, account.realm)){
        this.account = new SimpleAccount(account.principal, null, account.realm);
...
/*
    if(utils.isDefined(this.session.save)){
