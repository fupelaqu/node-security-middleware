...
/**
 * forEach executes the provided function (fn) once for each element 
 * present in the array or each property present in the object until it finds 
 * one where fn returns a false value.
 */
var forEach = function(obj, fn) {
    ...
    if(obj instanceof Array){
        ...
        for ( var i = 0; i < obj.length; i++) {
            if(fn.apply(null, [ obj[i], i, copy ].concat(fixedArgs)) === false){
                break;
            }
        }
    }
        ...
        for ( var property in obj) {
            if (Object.prototype.hasOwnProperty.call(obj, property)
                    && Object.prototype.propertyIsEnumerable
                            .call(obj, property)) {
                if (fn.apply(null, [ property, obj[property] ]
...
var asyncSeries = function (callbacks, check, last) {
    function next(value) {
        if(isDefined(value) && !check(value)){
            last(value);
        }
        else{
...
var async = function(obj, fn, callback, check){
    var fixedArgs = asArray(arguments, 4);    
    var callbacks = [];
    if(obj instanceof Array){
        var copy = [].concat(obj);
        var wrapper = function(obj, i, copy, fixedArgs){
            return function(next){
                fn.apply(null, [ next, obj[i], i, copy ].concat(fixedArgs));
            };
        };
        for ( var i = 0; i < obj.length; i++) {
            callbacks.push(wrapper(obj, i, copy, fixedArgs));
        }
    }
    else if (typeof obj === 'object'){
        var wrapper = function(obj, property){
            return function(next){
                fn.apply(null, [ next, property, obj[property] ].concat(fixedArgs));
            };
        };
        for ( var property in obj) {
            if (Object.prototype.hasOwnProperty.call(obj, property)
                && Object.prototype.propertyIsEnumerable.call(obj,
                    property))
                callbacks.push(wrapper(obj, property));
        }
    }
    asyncSeries(callbacks, check, function(value) {
        if(callback && 'function' === typeof callback){
            callback(value);
        }
    });
...
/**
 * forEachAsync executes the provided asynchronous function (fn) once for each element 
 * present in the array or each property present in the object until it finds 
 * one where fn returns a false value.
 */
var forEachAsync = function(obj, fn, callback) {
    async(obj, fn, callback, function(value) {
        return value !== false;
    });
};
...
/**
 * every executes the provided asynchronous function (fn) once for each element present 
 * in the array or each property present in the object until it finds one where 
 * callback returns a false value. 
 * If such an element is found, the every method immediately returns false. 
 * Otherwise, if callback returned a true value for all elements, every will return true. 
 */
var everyAsync = function(obj, fn, callback) {
    async(obj, fn, callback, function(value) {
        return value === true;
    });
};
...
/**
 * some executes the provided asynchronous function (fn) once for each element present in the 
 * array or each property present in the object until it finds one where 
 * callback returns a true value. 
 * If such an element is found, some immediately returns true. 
 * Otherwise, some returns false. 
 */
var someAsync = function(obj, fn, callback){
    async(obj, fn, callback, function(value) {
        return value !== true;
    });
};
